...
const CA_KEY_PATH =
	process.env.CA_KEY_PATH || "/data/certs/postgres-clients/ca.key"; // Only needed for signing
...
const CertManager = {
	...
	createCert: async (name) => {
		...
		try {
			...
			// I'll make CN = name.
			await execPromise(
				`openssl req -new -key ${keyPath} -out ${csrPath} -subj "/CN=${safeName}"`,
			);

			// 3. Sign CSR with CA
			// valid for 365 days
			await execPromise(
				`openssl x509 -req -in ${csrPath} -CA ${CA_CERT_PATH} -CAkey ${CA_KEY_PATH} -CAcreateserial -out ${crtPath} -days 365 -sha256`,
			);
