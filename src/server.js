...
app.use((req, res, next) => {
    ...
    if (ALLOWED_LOCALS.includes(clientIp) || ALLOWED_LOCALS.includes(req.connection.remoteAddress)) {
        return next();
    }
    ...
    res.status(403).send('Forbidden: Access allowed only from Tailscale network.');
});
...
app.get('/', async (req, res) => {
    try {
        ...
        try {
            ...
            console.warn("Could not fetch log history (maybe table missing?):", e.message);
        }
...
app.get('/databases', async (req, res) => {
    try {
        ...
        const usersResult = await db.query('SELECT rolname FROM pg_roles ORDER BY rolname');
...
app.get('/users', async (req, res) => {
    try {
        ...
        const systemUsers = allUsers.filter(u => u.name.trim().startsWith('pg_') || u.name.trim() === 'postgres');
        const regularUsers = allUsers.filter(u => !u.name.trim().startsWith('pg_') && u.name.trim() !== 'postgres');

        const dbsResult = await db.query('SELECT datname FROM pg_database WHERE datistemplate = false ORDER BY datname');
...
app.post('/users/create', async (req, res) => {
    try {
        const { name, password, superuser, createrole, createdb, canlogin, create_new_db, new_db_name, access_dbs } = req.body;
        ...
        if (canlogin) options.push("LOGIN"); else options.push("NOLOGIN");
        
        // Escape single quotes in password
        const safePassword = password.replace(/'/g, "''");
        
        // 1. Create User
        await db.query(`CREATE USER ${safeName} WITH PASSWORD '${safePassword}' ${options.join(' ')}`);
...
app.get('/certs/download/ca', async (req, res) => {
    try {
        const caPath = process.env.CA_CERT_PATH || '/data/certs/postgres-clients/ca.crt';
...
app.get('/certs/download/:name/:type', (req, res) => {
    ...
    if (!['crt', 'key'].includes(type)) return res.status(400).send('Invalid type');
...
app.get('/status', async (req, res) => {
    try {
        ...
        const tableCheck = await db.query("SELECT to_regclass('public.postgres_log') as exists");
...
app.post('/status/enable-logging', async (req, res) => {
    try {
        const initSqlPath = path.join(__dirname, '../postgres-config/init.sql');
        const sqlContent = require('fs').readFileSync(initSqlPath, 'utf8');
        ...
        for (const stmt of statements) {
            ...
            try {
                ...
                if (e.code === '42710') { // duplicate_object (e.g. server exists)
                    console.log("Object already exists, skipping:", stmt.substring(0, 50));
                } else if (e.code === '42704') { // undefined_object
                     // ignore
                     throw e; 
                } else {
                    console.warn(`Error running statement: ${stmt.substring(0, 50)}...`, e.message);
